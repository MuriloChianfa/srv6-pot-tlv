...
#pragma clang loop unroll(full)
    ...
    __u32 seg_cnt = srh->segments_left + 1;
    if (seg_cnt > SRH_MAX_ALLOWED_SEGMENTS) {
        bpf_printk("[seg6_pot_tlv][-] max seg_cnt reached");
        return -1;
    }
    ...
    __u8 leaves[SRH_MAX_ALLOWED_SEGMENTS][BLAKE3_DIGEST_LEN];
    compute_merkle_leaves(skb, srh, seg_cnt, &leaves, &tlv.nonce, data, end);
    dump_tlv_nonce(leaves);
